...
def simple_describe_lr(fit):
    ...
    results += r"<tr><th> $\beta_0$ </th><td> {0} </td></tr>".format(
        fit["coefficients"][0]
    )

    variables = fit["formula"].split("~")[1].split("+")
    for i, b in enumerate(zip(variables, fit["coefficients"][1:])):
        results += r"<tr><td> {1} ($\beta_{0}$) </td><td> {2} </td></tr>".format(
            i + 1, b[0], b[1]
        )
...
def describe_bootstrap_lr(fit, significant_digits=2):
    ...
    results += (
        ...
        + str(sd)
        + "f})</td></tr>"
    ).format(fit["coefficients"][0], bounds[0][0], bounds[0][1])

    variables = fit["formula"].split("~")[1].split("+")
    for i, b in enumerate(zip(variables, fit["coefficients"][1:], bounds[1:])):
        results += (
            r"<tr><td> {1} ($\beta_{0}$) </td><td> {2:."
            + str(sd)
            + r"f} </td><td>({3:."
            + str(sd)
            + r"f}, {4:."
            + str(sd)
            + r"f})</tr>"
        ).format(i + 1, b[0], b[1], b[2][0], b[2][1])
    ...
    results += (
        ...
        + str(sd)
        + r"f}) </td></tr>"
    ).format(fit["sigma"], sigma_bounds[0], sigma_bounds[1])
    results += (
        r"<tr><th> $R^2$ </th><td> {0:."
        + str(sd)
        + r"f} </td><td>({1:."
        ...
        + str(sd)
        + r"f})</td></tr>"
    ).format(fit["r_squared"], r_bounds[0], r_bounds[1])
    results += r"</table>"
    return HTML(results)
...
def describe_bootstrap_lgr(fit, significant_digits=2):
    ...
    for i, b in enumerate(zip(variables, fit["coefficients"][1:], bounds[1:])):
        results += (
            ...
            + str(sd)
            + r"f}</td></tr>"
        ).format(i + 1, b[0], b[1], b[2][0], b[2][1], b[1] / 4)
    ...
    results += (
        ...
        + str(sd)
        + r"f}) </td></tr>"
    ).format(fit["error"], sigma_bounds[0], sigma_bounds[1])
    results += (
        r"<tr><th> $R^2$ </th><td> {0:."
        + str(sd)
        + r"f} </td><td>({1:."
