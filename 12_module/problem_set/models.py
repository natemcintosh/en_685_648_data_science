...
def logistic_regression(formula, data=None):
    ...
    y = np.ravel(
        y
    )  # not sure why this is needed for LogisticRegression but not LinearRegression
...
def bootstrap_linear_regression(
    formula, data=None, samples=100, style="linear", params={}
):
    if data is None:
        raise ValueError(
            "The parameter 'data' must be assigned a non-nil reference to a Pandas DataFrame"
    ...
    for i in range(samples):
        sampling_indices = [
            i for i in [np.random.randint(0, n - 1) for _ in range(0, n)]
        ]
    ...
    if bootstrap:
        ...
        metrics = [
            [
                error,
                fmt(fit["sigma"], sd),
                fmt(sigma_bounds[0], sd),
                fmt(sigma_bounds[1], sd),
            ],
            [
                r_label,
                fmt(fit["r_squared"], sd),
                fmt(r_bounds[0], sd),
                fmt(r_bounds[1], sd),
            ],
        ]
    else:
        metrics = [[error, fmt(fit["sigma"], sd)], [r_label, fmt(fit["r_squared"], sd)]]
...
class ResultsView(object):
    ...
    def _repr_html_(self):
        ...
        if self.bootstrap and self.is_logistic:
            span = "5"
        result = (
            r"<table><tr><th colspan=" + span + r">Linear Regression Results</th></tr>"
        )
        if self.is_logistic:
            result = (
                r"<table><tr><th colspan="
                + span
                + r">Logistic Regression Results</th></tr>"
            )
    ...
    def _repr_latex_(self):
        ...
        if self.bootstrap and self.is_logistic:
            span = 4
        result = (
            r"\begin{table}[!htbp] \begin{tabular}{"
            + (r"l" * span)
            + r"} \hline \multicolumn{"
            + str(span)
            + r"}{c}{\textbf{Linear Regression}} \\ \hline \hline "
        )
        if self.is_logistic:
            result = (
                r"\begin{table}[!htbp] \begin{tabular}{"
                ...
                + r"} \hline \multicolumn{"
                + str(span)
                + r"}{c}{\textbf{Logistic Regression}} \\ \hline \hline "
            )

        result += (
            r"\multicolumn{"
            + str(span)
            + r"}{l}{\textbf{Coefficients}}        \\ \hline "
        )
        coefficients = self.content["coefficients"]
        template = r""
        headers = r""
